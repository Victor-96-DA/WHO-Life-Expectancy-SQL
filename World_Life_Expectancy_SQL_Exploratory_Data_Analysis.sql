...
ORDER BY Life_Expectancy_Increase_Over_Years 
LIMIT 10;
# Insights: The 10 countries that improved the least are developing countries but they already had a relatively high
-- life expectancy (having the minimum as 65 and 78.2 as the maximum) and this suggests less room for growth.
...
-- 2 showed stagnant process( Brazil and Somalia at 0); The stagnancy for Somalia could be 
-- attached to the conflict, instablity and weak healthcare infrastructure.
-- Why did Brazil record a stagnant life expectancy? An OUTLIER? Brazil,  although has a large economy with a GDP of about 98293, 
-- it recorded 2411 Ault Mortality rates over the years 
...
-- Conclusion: 11 countries showed stagnant or declining life expectancy over the study period. Conflict-affected nations 
-- (Syria, Libya, Yemen, Iraq) show the strongest declines, suggesting war is a more significant driver of life expectancy 
-- loss than economic or health factors alone.
...
-- Cnnclusion:  strong positive correlation exists between GDP and life expectancy. Countries with average GDP between 10,000 and 58,000 
-- consistently recorded life expectancy in the 70s and 80s, while countries with average GDP between 137 and 365 recorded life expectancy in the 40s to 60s; 
-- a difference of roughly 20-30 years of life.
...
-- 151 as against 32 stands as an outlier but it is a solid proof that the developed countries, athough few, are a 
-- group of healthy nation with great GDPs and other advanced infrastructure that helps healthy lifestyle, 
-- 151 on the other hand ranges from middle income countries to relatively poor countries and this puls down their average significantly. 
-- Conclusion:  The comparison between Developed and Developing countries should be interpreted cautiously , 
-- the significant imbalance in group sizes (32 vs 151) and the high variance within the Developing category may influence the average gap of 12 years.
...
-- Countries averaging 14+ years of schooling consistently record life expectancy in the 70s and 80s, while countries below 8 years average between 48-65; 
-- confirming education as a significant factor in population health outcomes.
-- Although countries like Ukraine have have schooling years of 14.6 and yet the avg_life_exp 69.9; a proof that  war seems to have a 
-- strong negative impact on life expectancy even where there are advanced healthcare infrastructure and high GDP, they are all destroyed by war. 
...
FROM world_life_expectancy
WHERE `Life Expectancy` IS NOT NULL
AND `percentage expenditure` IS NOT NULL
AND `bmi` IS NOT NULL
GROUP BY country
...
-- Conclusion: Adult mortality is consistent across all low performing countries, suggesting that preventable adult deaths are the strongest 
-- driver of low life expectancy in developing nations.
...
#RECOMMENDATIONS
-- Economic Development: Governments should prioritize economic growth and equitable income distribution. 
-- The strong correlation between GDP and life expectancy suggests that improving citizens' standard of living directly translates to longer, healthier lives.

-- Healthcare Infrastructure: Investment in accessible and advanced healthcare infrastructure is critical, 
-- particularly in Sub-Saharan African nations where adult mortality and infant deaths are highest. 
-- Reducing adult mortality is identified as the single most consistent factor separating low and high performing countries.

-- Education Access: Education should be made universally accessible and affordable. 
-- Countries averaging 15+ years of schooling consistently recorded life expectancy in the 70s and 80s, 
-- suggesting education is as important as economic factors in determining health outcomes

-- Global Peace and International Relations: Conflict is a direct destroyer of life expectancy gains; 
-- Syria, Yemen, Iraq and Libya all recorded declining life expectancy directly linked to war and instability. 
-- International diplomacy and peacekeeping efforts are therefore as critical as health policy in improving global life expectancy.
